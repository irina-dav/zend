...
url_articles =          url_api + 'help_center/incremental/articles.json?start_time={start_time}&include=sections'
url_article_comments =  url_api + 'help_center/ru/articles/{article_id}/comments.json'
...
def get_start_date():
    with shelve.open(config.shelve_name) as db:
        start_date = db.get('start_date', datetime.now().astimezone(tzu) - timedelta(days=7))
    return start_date
...
def fetch_data(url):
    ...
    if response.status_code != 200:
        logging.error(f'Response status: {response.status_code}. Problem with the request. Exiting')
        return None
    return response.json()

def send_to_telegram(*messages):
    try:
        updater = Updater(token=config.token, request_kwargs=config.bot_request_args)
        bot = updater.bot
        message = '\n\n'.join([m for m in messages if m])
        if message:
            bot.sendMessage(chat_id=config.channel_id, text=message, parse_mode=ParseMode.HTML, disable_web_page_preview=True)
...
def get_z_objects(api_url, cls, start_date):
    ...
    if data_objects['count'] == 0: 
        ...
        if cls is Article:
            sections_dict = {s['id']: s['name'] for s in data_objects['sections']}
            objects = [Article(a, sections_dict) for a in data_objects['articles']]
        elif cls is Post:
            topics_dict = {t['id']: t['name'] for t in data_objects['topics']}
            objects = [Post(p, topics_dict) for p in data_objects['posts'] if parse(p['updated_at']) > start_date]
    ...
    for o in objects_upd:
        if cls is Article:
            data_comments = fetch_data(url_article_comments.format(article_id=o.id))
        elif cls is Post:
            data_comments = fetch_data(url_post_comments.format(post_id=o.id))

        if any(parse(comm['updated_at']) > start_date for comm in data_comments['comments']):
            objects_with_new_comm.append(o)

    logging.info(f'{cls_name}s, with new comments count: {len(objects_with_new_comm)}')

    return (objects_new, objects_with_new_comm)

if __name__ == '__main__':
    html_articles = html_articles_new_comm = html_posts_new = html_posts_new_comm = None
    start_date = get_start_date()
    start_date_unix = int(time.mktime(start_date.timetuple()))
    logging.info(f'Start date: {start_date}')

    articles_new, articles_with_new_comm = get_z_objects(url_articles.format(start_time=start_date_unix), Article, start_date)
   
    if articles_new and len(articles_new) > 0:
        html_articles = '<b>Обновления в статьях:</b>\n\n' + '\n\n'.join(
            [f'[{a.section}]\n{a.title}<a href="{a.html_url}">\nЧитать статью</a>' 
                for a in articles_new])

    if articles_with_new_comm and len(articles_with_new_comm) > 0:
        html_articles_new_comm = '<b>Cтатьи с новыми комментариями:</b>\n\n' + '\n\n'.join(
            [f'[{a.section}]\n{a.title}<a href="{a.html_url}">\nЧитать статью</a>' 
    ...
    if posts_new and len(posts_new) > 0:
        html_posts_new = '<b>Новые посты:</b>\n\n' + '\n\n'.join(
            [f'[{p.topic}]\n{p.title}<a href="{p.html_url}">\nЧитать пост</a>'
                for p in posts_new])
    
    if posts_with_new_comm and len(posts_with_new_comm) > 0:
        html_posts_new_comm = '<b>Посты с новыми комментариями:</b>\n\n' + '\n\n'.join(
            [f'[{p.topic}]\n{p.title}<a href="{p.html_url}">\nЧитать пост</a>'
                for p in posts_with_new_comm])

    send_to_telegram(html_articles, html_articles_new_comm, html_posts_new, html_posts_new_comm)
